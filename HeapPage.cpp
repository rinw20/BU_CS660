...
////    return this->slot != other.slot || *this->page != *other.page;
//    if(this->slot!=other.slot){
//        return true;
//    }
//    return false;
    return !(slot == other.slot && page->pid.pageNumber() == other.page->pid.pageNumber() && page->pid.getTableId() == other.page->pid.getTableId());
}

HeapPageIterator &HeapPageIterator::operator++() {

    if(slot>=page->numSlots){
        return *this;
    }
    do {
        slot++;
...
int HeapPage::getNumTuples() {
    ...
    double page_size = Database::getBufferPool().getPageSize();
    double tuple_size = this->td.getSize();
    return (int)std::floor((page_size * 8) / (tuple_size * 8 + 1));
}
...
int HeapPage::getHeaderSize() {
    ...
    double page_size = Database::getBufferPool().getPageSize();
    double tuple_size = this->td.getSize();
    return page_size - tuple_size * getNumTuples();
}
...
bool HeapPage::isSlotUsed(int i) const {
    ...
    int offset = i % 8;
    int temp = 1 << offset;
    return (this->header[byteLoc] & temp) != 0;
}
...
//        slot++;
//    }
    return HeapPageIterator(slot, this);
}
